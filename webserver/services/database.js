...
function execProcDynamicNoOutRec(plsqlProc, binds1 = {},recName) {
  return new Promise(async (resolve, reject) => {
  ...
  try {
      ...
      const p_param= new RectypeClass(binds1.p_in)

    let binds = {
        inbv: p_param
        ,outbv: { type: oracledb.STRING, dir: oracledb.BIND_OUT }
    };
      ...
      // conn = await oracledb.getConnection();
      const result = await conn.execute(plsqlProc, binds);
      resolve(result);
